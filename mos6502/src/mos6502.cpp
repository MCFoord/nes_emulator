...
void mos6502::Write(uint16_t addr, uint8_t value)
{
...
void mos6502::run(std::ostream& output, int numOperations)
{
	uint16_t operationAddr;
	while (!illegalOpcode && numOperations > 0)
	{
		...
		// execute
		Exec(instr);
		printOperation(operationAddr, output);
		--numOperations;
	}
}
